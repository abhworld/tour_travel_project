/**
 * @file pdfuploads plugin for CKEditor
 *	Version: 4.3.6
...
function checkDimension(ev)
{
	var data = ev.data,
		...
		maximum = config.pdfuploads_maximumDimensions;

	var img = data.image;

	if (maximum.width && img.width > maximum.width)
	{
		//alert(editor.lang.pdfuploads.imageTooWide);
		errormsg_dlg(editor.lang.pdfuploads.imageTooWide, 'File Upload');
		ev.cancel();
		return;
	}
	...
	{
		errormsg_dlg(editor.lang.pdfuploads.imageTooTall, 'File Upload');
		ev.cancel();
		return;
	}
}
...
var htmlFilterRules = {
	elements: {
		$: function( element ) {
			...
			// remove our wrappers
			if ( className == "pdfuploadsTmpWrapper" )
				return false;
		}
	}
};
...
function getEditorCss( config ) {
	var imgUpload = 'span.pdfuploadsTmpWrapper>span { top: 50%; margin-top: -0.5em; width: 100%; text-align: center; color: #fff; ' +
		'text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5); font-size: 50px; font-family: Calibri,Arial,Sans-serif; pointer-events: none; ' +
		'position: absolute; display: inline-block;}';
	if (config.pdfuploads_hideImageProgress)
		imgUpload = 'span.pdfuploadsTmpWrapper { color:#333; background-color:#fff; padding:4px; border:1px solid #EEE;}';

	return '.pdfuploadsOverEditor { ' + (config.pdfuploads_editorover || 'box-shadow: 0 0 10px 1px #999999 inset !important;') + ' }' +
		'a.pdfuploadsTmpWrapper { color:#333; background-color:#fff; padding:4px; border:1px solid #EEE;}' +
		'.pdfuploadsTmpWrapper { display: inline-block; position: relative; pointer-events: none;}' +
		imgUpload +
		...
		'.pdfuploadsTmpWrapper .uploadCancel { background-color: #333333;border-radius: 0.5em;color: #FFFFFF;cursor: pointer !important;' +
		...
		'.pdfuploadsTmpWrapper span uploadCancel { width:1em; padding-left:0}';
}
...
function PickAndSendFile(editor, forImage, caller, callback) {
	...
	{
		errormsg_dlg("Please, wait to finish the current upload", 'File Upload');
		return;
	}
	...
	{
		// old IE
		var iframe = document.getElementById("pdfuploadsTarget");
		if (!iframe) {
			iframe = document.createElement("iframe");
			iframe.style.display = "none";
			iframe.id = "pdfuploadsTarget";
		...
		var fnNumber = editor._.pdfuploadsFormUploadFn;
		var fnInitPicker = editor._.pdfuploadsFormInitFn;
		if (!fnNumber)
		{
			editor._.pdfuploadsFormUploadFn = fnNumber = CKEDITOR.tools.addFunction( setUrl, editor );
			editor._.pdfuploadsFormInitFn = fnInitPicker = CKEDITOR.tools.addFunction( function() {
				window.setTimeout(function() {
					var picker = document.getElementById("pdfuploadsTarget").contentWindow.document.getElementById("upload");
					picker.onchange=function() {
						var evdata = {
							...
							requiresVideo: IEUpload_forVideo
						};
						...
						var result = filePickerEditor.fire("pdfuploads.startUpload", evdata );
						// in v3 cancel() returns true and in v4 returns false
						// if not canceled it's the evdata object
						if ( typeof result == "boolean" )
							return;

						if (evdata.requiresImage && !CKEDITOR.plugins.pdfuploads.isImageExtension(filePickerEditor, evdata.name))
						{
							errormsg_dlg(filePickerEditor.lang.pdfuploads.nonImageExtension, 'File Upload');
							return;
						}
			...
			editor.on( "destroy", function () {
				CKEDITOR.tools.removeFunction( this._.pdfuploadsFormUploadFn );
				CKEDITOR.tools.removeFunction( this._.pdfuploadsFormInitFn );
			} );
		}
		...
		iframe.onreadystatechange = function() {
			if (iframe.readyState == "complete") {
				window.setTimeout(function() {
					if (IEUpload_fileName)
					{
						errormsg_dlg("The file upload has failed", 'File Upload');
	...
	if (!filePicker)
	{
		...
		filePicker.addEventListener("change", function ()
		{
			...
			for(var i=0; i<count; i++)
			{
				...
				evData.id = CKEDITOR.plugins.pdfuploads.getTimeStampId();
				...
				CKEDITOR.plugins.pdfuploads.insertSelectedFile(filePickerEditor, evData);
			}
		});
	}
	...
	function setUrl( fileUrl, msg )
	{
		...
		if ( typeof msg == "string" && msg && !fileUrl)
			errormsg_dlg( msg, 'File Upload' );

		var editor = filePickerEditor;
		editor.fire('pdfuploads.endUpload', { name: IEUpload_fileName, ok: (!!fileUrl) } );
		...
		if ( fileUrl ) {
			...
			else
			{
				...
				element.on("load", function(e) {
					...
					editor.fire('pdfuploads.finishedUpload', { name: IEUpload_fileName, element: element } );
				});
			...
			if (filePickerForceLink)
				filePickerEditor.fire("pdfuploads.finishedUpload", { name: IEUpload_fileName, element: element } );
		}
...
function receivedUrl(fileUrl, data, editor, el, attribute)
{
	...
	editor.fire('pdfuploads.finishedUpload', { name: data.name, element: el } );
}

CKEDITOR.plugins.add( "pdfuploads",
{
	...
	onLoad : function()	{
		...
		var content = '.pdfuploadsOverContainer {' + (CKEDITOR.config.pdfuploads_containerover || 'box-shadow: 0 0 10px 1px #99DD99 !important;') + '} ' +
					'.pdfuploadsOverDialog {' + (CKEDITOR.config.pdfuploads_dialogover || 'box-shadow: 0 0 10px 4px #999999 inset !important;') + '} ' +
					'.pdfuploadsOverCover {' + (CKEDITOR.config.pdfuploads_coverover || 'box-shadow: 0 0 10px 4px #99DD99 !important;') + '} ';
	...
	init : function( editor ) {
		...
		if (typeof config.pdfuploads_imageExtensions == "undefined")
			config.pdfuploads_imageExtensions = "jpe?g|gif|png";
		...
		if (!config.filebrowserUploadUrl && !config.filebrowserImageUploadUrl)
		{
			if (window.console && console.log)
			{
				...
				console[ console.warn ? "warn" : "log" ]("The 'pdfuploads' plugin now is disabled.");
			}
			return;
		}
		...
		{
			editor.addFeature( {
				allowedContent: 'img[!src,width,height];a[!href];span[id](pdfuploadsTmpWrapper);'
			} );
		}
		...
		editor.on( "pdfuploads.startUpload" , function(ev)
		{
			var editor = ev.editor,
				config = editor.config,
				file = ev.data && ev.data.file;
			if (config.pdfuploads_maxFileSize &&
				file && file.size &&
				file.size > config.pdfuploads_maxFileSize )
			{
				//alert( editor.lang.pdfuploads.fileTooBig );
				errormsg_dlg( editor.lang.pdfuploads.fileTooBig, 'File upload limit exceed' );
				ev.cancel();
			}
			var name = ev.data.name;
			if (config.pdfuploads_invalidExtensions)
			{
				var reInvalid = new RegExp( "\.(?:" + config.pdfuploads_invalidExtensions + ")$", "i");
				if ( reInvalid.test( name ) )
				{
					errormsg_dlg( editor.lang.pdfuploads.invalidExtension, 'File Upload' );
					ev.cancel();
				}
			}
			if (config.pdfuploads_acceptedExtensions)
			{
				var reAccepted = new RegExp( "\.(?:" + config.pdfuploads_acceptedExtensions + ")$", "i");
				if ( !reAccepted.test( name ) )
				{
					errormsg_dlg( editor.lang.pdfuploads.nonAcceptedExtension.replace("%0", config.pdfuploads_acceptedExtensions), 'File Upload' );
					ev.cancel();
				}
			}
		});

		// Special listener that captures uploads of images and if there's some listener set for "pdfuploads.localImageReady"
		...
		editor.on( 'pdfuploads.startUpload' , function(ev) {
			...
			if (data.forceLink || !CKEDITOR.plugins.pdfuploads.isImageExtension(editor, data.name))
				return;
			...
			if (!editor.hasListeners( 'pdfuploads.localImageReady' ))
				return;
			...
			if (data.mode.type=="base64paste")
			{
				...
				var idTmp = CKEDITOR.plugins.pdfuploads.getTimeStampId();
				data.result = "<span id='" + idTmp + "' class='pdfuploadsTmpWrapper' style='display:none'>&nbsp;</span>";
				data.mode.id = idTmp;
			}
			...
			img.onload = function() {
				...
				var result = editor.fire('pdfuploads.localImageReady', evData);
				...
				CKEDITOR.plugins.pdfuploads.insertProcessedFile(ev.editor, evData);
			};
		...
		if (config.pdfuploads_convertBmp)
			editor.on( 'pdfuploads.localImageReady', convertToBmp);

		if (config.pdfuploads_maximumDimensions)
			editor.on( 'pdfuploads.localImageReady', checkDimension);


		// workaround for image2 support
		 editor.on( 'pdfuploads.finishedUpload' , function(ev) {
		...
		editor.on( "paste", function(e) {
			...
			{
				errormsg_dlg("Sorry, the images pasted with Safari aren't usable", 'File Upload');
				...
				html = html.replace( /<img(.*?) src="data:image\/.{3,4};base64,.*?"(.*?)>/g, function( img )
					{
						...
						var match = img.match(/"(data:image\/(.{3,4});base64,.*?)"/),
							...
							id = CKEDITOR.plugins.pdfuploads.getTimeStampId();
		...
		var avoidBadUndo = function(e) {
			...
			if (root.$.querySelector( ".pdfuploadsTmpWrapper") )
			{
		...
		editor.addCommand( 'addPDF', {
			exec: function( editor ) {
				PickAndSendFile(editor, false, this);
			}
		});

		editor.ui.addButton( 'addPDF', {
			label: editor.lang.pdfuploads.addFile,
			command: 'addPDF',
			icon : this.path + 'icons/addfile.png', // %REMOVE_LINE_CORE%
			toolbar: 'insert',
			allowedContent : 'a[!href];span[id](pdfuploadsTmpWrapper);',
			requiredContent : 'a[href]'
		}); 

		// Images
		/*
		editor.addCommand( 'addPDFImage', {
			exec: function( editor ) {
				PickAndSendFile(editor, true, this);
			}
		});

		editor.ui.addButton( 'addPDFImage', {
			label: editor.lang.pdfuploads.addImage,
			command: 'addPDFImage',
			icon : this.path + 'icons/addimage.png', // %REMOVE_LINE_CORE%
			toolbar: 'insert',
			allowedContent : 'img[!src,width,height];span[id](pdfuploadsTmpWrapper);',
		...
		*/
		...
		var removeBaseHighlight = function() {
			...
			if ( dialog )
			{
				...
				div.removeClass( 'pdfuploadsOverCover' );
			}
			else
			{
				editor.container.removeClass( 'pdfuploadsOverContainer' );
			}
		};

		editor.on( 'destroy', function() {
			CKEDITOR.removeListener( 'pdfuploads.droppedFile', removeBaseHighlight);
		...
		CKEDITOR.on( 'pdfuploads.droppedFile', removeBaseHighlight);
		...
		var CKEDITORdragenter = function(e) {
			if (mainMinX == -1)
			{
				...
				if ( dialog )
				{
					...
					var div = dialog.parts.title.getParent();
					div.addClass( 'pdfuploadsOverCover' );
				}
				else
				{
					if (!editor.readOnly)
						editor.container.addClass( 'pdfuploadsOverContainer' );
				}
		...
		var rootDropListener = function(e) {
			// editor
			visibleRoot.removeClass( "pdfuploadsOverEditor" );
			minX = -1;
			...
			CKEDITOR.fire( 'pdfuploads.droppedFile' );
			mainMinX = -1;
			...
			if ( data && data.files && data.files.length>0 )
			{
				...
				for( var i=0; i<data.files.length; i++)
				{
					...
					CKEDITOR.plugins.pdfuploads.insertDroppedFile(editor, evData);
				}
			}
		};
		...
		var rootDragEnter = function(e) {
			if (minX == -1)
			{
				...
				if (!editor.readOnly)
					visibleRoot.addClass( "pdfuploadsOverEditor" );
			...
			addTarget : function( target, callback )
			{
				target.on( 'dragenter', function(e) {
					if (minX == -1)
					{
						...
						target.addClass( 'pdfuploadsOverDialog' );
				...
				target.on( 'dragleave', function(e) {
					...
					if ((ev.clientX<=minX) || (ev.clientY<=minY) || (ev.clientX>=maxX) || (ev.clientY>=maxY))
					{
						target.removeClass( 'pdfuploadsOverDialog' );
						minX = -1;
					}
				});
				...
				target.on( 'drop', function(e) {
					...
					if ( data && data.files && data.files.length>0 )
					{
						...
						for( var i=0; i<1; i++)
						{
							...
							CKEDITOR.plugins.pdfuploads.processFileWithCallback( editor, evData );
						}
					}
				});
			}
...
CKEDITOR.plugins.pdfuploads = {
	...
	{
		if (!editor.config.pdfuploads_imageExtensions)
			return false;

		var imageRegexp = new RegExp( "\.(?:" + editor.config.pdfuploads_imageExtensions + ")$", "i");
	...
	insertProcessedFile: function(editor, evData) {
		...
		switch (evData.mode.type)
		{
			...
			default:
				errormsg_dlg("Error, no valid type", evData.mode);
				break;
		}
	},
...
{
	var isImage = CKEDITOR.plugins.pdfuploads.isImageExtension( editor, data.name ),
		showImageProgress = !editor.config.pdfuploads_hideImageProgress,
		element;
	...
	else
	{
		...
		element.setAttribute( "class", "pdfuploadsTmpWrapper");
...
{
	e.removeListener();
	errormsg_dlg("Failed to load the image with the provided URL: '" + e.sender.data( 'cke-saved-src') + "'", 'File Upload');
...
function checkLoadedImage(img, editor, el, name)
{
	...
	editor.fire('pdfuploads.finishedUpload', { name: name, element: img } );
...
function createXHRupload(editor, data)
{
	...
	{
		errormsg_dlg(editor.lang.pdfuploads.nonImageExtension, 'File Upload');
		return null;
	}

	var result = editor.fire("pdfuploads.startUpload", data);
	// in v3 cancel() returns true and in v4 returns false
...
//			data.xhr = reader;
	...
	xhr.onload = function() {
            ...
            var result = editor.fire("pdfuploads.serverResponse", evtData);
            ...
            editor.fire('pdfuploads.endUpload', { name: data.name, ok: (!!fileUrl), xhr : xhr, data : data } );
            if (xhr.status!=200)
            {
                if (xhr.status == 413)
                    errormsg_dlg( editor.lang.pdfuploads.fileTooBig, 'File Upload' );
                else
                    errormsg_dlg('Error posting the file to ' + data.url + '\r\nResponse status: ' + xhr.status, 'File Upload');
	...
	xhr.onerror = function(e) {
		errormsg_dlg('Error posting the file to ' + data.url, 'File Upload' );
		if (window.console)
			console.log(e);
...
function updateProgress(editor, id, evt)
{
	...
	{
		textValue = editor.lang.pdfuploads.processing;
		value = "100%";
	/*
		if (text)
		{
...
function createSVGAnimation( file, id, editor )
{
	...
	function createPdfUpload( editor, dialogName, definition, element )
	{
		...
		if (element.filebrowser.action == "QuickUpload") {
			...
			if (!editor.config.pdfuploads_respectDialogUploads)
			{
				element.label = ( imageDialog ? editor.lang.pdfuploads.addImage : editor.lang.pdfuploads.addFile);
	...
	function applySimpleUpload( editor, dialogName, definition, elements )
	{
		for ( var i in elements )
		{
			...
			if ( element.filebrowser && element.filebrowser.url )
				createPdfUpload( editor, dialogName, definition, element );
		}
	}
	...
	function processPastedFile(file, e)
	{
		...
		var id = CKEDITOR.plugins.pdfuploads.getTimeStampId(),
		...
		CKEDITOR.plugins.pdfuploads.insertPastedFile(editor, evData);
	}
	...
	{
		if (!evt.editor.plugins.pdfuploads)
			return;
...
/**
 * Fired when file starts being uploaded by the "pdfuploads" plugin
 *
 * @since 3.1
 * @name CKEDITOR.editor#pdfuploads.startUpload
 * @event
 ...
 *
 * @since 4.3.6
 * @name CKEDITOR.editor#pdfuploads.serverResponse
 * @event
 * @param {Object} [xhr] The XHR with the request.
 * @param {Object} [data] The original data object of this upload.
 * Upon processing this event, a listener can set a "url" property on the event.data object and that will tell to the pdfuploads plugin
...
/**
 * Fired when file upload ends on the "pdfuploads" plugin
 *
 * @since 3.1
 * @name CKEDITOR.editor#pdfuploads.endUpload
 * @event
 * @param {String} [name] The file name.
...
/**
 * Fired when the final element has been inserted by the "pdfuploads" plugin (after it has been uploaded)
 *
 * @since 3.3.4
 * @name CKEDITOR.editor#pdfuploads.finishedUpload
 * @event
 * @param {String} [name] The file name.
 ...
 * that contains the selected file. Extends the data provided in the pdfuploads.startUpload event
 * @since 4.2
 * @name CKEDITOR.editor#pdfuploads.localImageReady
 * @event
 ...
 *
 *		CKEDITOR.config.pdfuploads_containerover='border:1px solid red !important;';
 *
 * @since 2.7
 * @cfg {String} [pdfuploads_containerover='box-shadow: 0 0 10px 1px #99DD99 !important;']
 ...
 *
 *		CKEDITOR.config.pdfuploads_editorover='background-color:yellow !important;';
 *
 * @since 2.7
 * @cfg {String} [pdfuploads_editorover='box-shadow: 0 0 10px 1px #999999 inset !important;']
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pdfuploads_coverover='border:1px solid red !important;';
 *
 * @since 4.0
 * @cfg {String} [pdfuploads_coverover='box-shadow: 0 0 10px 4px #999999 inset !important;']
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pdfuploads_dialogover='border:1px solid red !important;';
 *
 * @since 4.0
 * @cfg {String} [pdfuploads_dialogover='box-shadow: 0 0 10px 4px #999999 inset !important;']
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pdfuploads_imageExtensions='jpe?g';
 *
 * @since 3.3.3
 * @cfg {String} [pdfuploads_imageExtensions='jpe?g|gif|png']
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pdfuploads_maxFileSize=10*1024*1024; // 10 Mb
 *
 * @since 3.3.3
 * @cfg {Number} [pdfuploads_maxFileSize=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pdfuploads_invalidExtensions='exe|php';
 *
 * @since 3.3.3
 * @cfg {String} [pdfuploads_invalidExtensions=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pdfuploads_acceptedExtensions='jpg|png|pdf|zip';
 *
 * @since 3.3.3
 * @cfg {String} [pdfuploads_acceptedExtensions=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pdfuploads_respectDialogUploads=true;
 *
 * @since 4.0
 * @cfg {Boolean} [pdfuploads_respectDialogUploads=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pdfuploads_hideImageProgress=true;
 *
 * @since 4.1
 * @cfg {Boolean} [pdfuploads_hideImageProgress=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pdfuploads_convertBmp=true;
 *
 * @since 4.2
 * @cfg {Boolean} [pdfuploads_convertBmp=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pdfuploads_maximumDimensions={width:500, height:400};
 *
 * @since 4.2
 * @cfg {Object} [pdfuploads_maximumDimensions=null]
 * @member CKEDITOR.config
 */
