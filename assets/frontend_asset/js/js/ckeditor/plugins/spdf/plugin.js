CKEDITOR.plugins.add('spdf', {
    ...
    init: function (editor) {
        editor.addCommand('spdf', {
            exec: function (editor) {
                a = document.createElement('input')
                a.setAttribute('type', 'file')
                a.setAttribute('accept', '.pdf')
                a.click()
                a.onchange = function () {
                    file = a.files[0];
                    ...
                    if (file.size > 5000000) {
                        ...
                        return
                    } else if (['pdf'].indexOf(file.type.split('/')[1]) === -1) {
                        b = document.createElement('div')
                        b.className = 'message alert alert-danger'
                        m = document.createElement('span')
...
//                                    imgElem = '<img src="' + url + '" class="image-editor" data-width="' + inputWidth + '" data-height="' + inputHeight + '" height="' + height + '" width="' + width + '">'
        ...
        editor.ui.addButton('SPdf', {
            label: 'Custom PDF Uploader',
            command: 'spdf',
            toolbar: 'insert'
        });
    }
