/**
 * @file pptuploads plugin for CKEditor
 *	Version: 4.3.6
...
function checkDimension(ev)
{
	var data = ev.data,
		...
		maximum = config.pptuploads_maximumDimensions;

	var img = data.image;

	if (maximum.width && img.width > maximum.width)
	{
		//alert(editor.lang.pptuploads.imageTooWide);
		errormsg_dlg(editor.lang.pptuploads.imageTooWide, 'File Upload');
		ev.cancel();
		return;
	}
	...
	{
		errormsg_dlg(editor.lang.pptuploads.imageTooTall, 'File Upload');
		ev.cancel();
		return;
	}
}
...
var htmlFilterRules = {
	elements: {
		$: function( element ) {
			...
			// remove our wrappers
			if ( className == "pptuploadsTmpWrapper" )
				return false;
		}
	}
};
...
function getEditorCss( config ) {
	var imgUpload = 'span.pptuploadsTmpWrapper>span { top: 50%; margin-top: -0.5em; width: 100%; text-align: center; color: #fff; ' +
		'text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5); font-size: 50px; font-family: Calibri,Arial,Sans-serif; pointer-events: none; ' +
		'position: absolute; display: inline-block;}';
	if (config.pptuploads_hideImageProgress)
		imgUpload = 'span.pptuploadsTmpWrapper { color:#333; background-color:#fff; padding:4px; border:1px solid #EEE;}';

	return '.pptuploadsOverEditor { ' + (config.pptuploads_editorover || 'box-shadow: 0 0 10px 1px #999999 inset !important;') + ' }' +
		'a.pptuploadsTmpWrapper { color:#333; background-color:#fff; padding:4px; border:1px solid #EEE;}' +
		'.pptuploadsTmpWrapper { display: inline-block; position: relative; pointer-events: none;}' +
		imgUpload +
		...
		'.pptuploadsTmpWrapper .uploadCancel { background-color: #333333;border-radius: 0.5em;color: #FFFFFF;cursor: pointer !important;' +
		...
		'.pptuploadsTmpWrapper span uploadCancel { width:1em; padding-left:0}';
}
...
function PickAndSendFile(editor, forImage, caller, callback) {
	...
	{
		// old IE
		var iframe = document.getElementById("pptuploadsTarget");
		if (!iframe) {
			iframe = document.createElement("iframe");
			iframe.style.display = "none";
			iframe.id = "pptuploadsTarget";
		...
		var fnNumber = editor._.pptuploadsFormUploadFn;
		var fnInitPicker = editor._.pptuploadsFormInitFn;
		if (!fnNumber)
		{
			editor._.pptuploadsFormUploadFn = fnNumber = CKEDITOR.tools.addFunction( setUrl, editor );
			editor._.pptuploadsFormInitFn = fnInitPicker = CKEDITOR.tools.addFunction( function() {
				window.setTimeout(function() {
					var picker = document.getElementById("pptuploadsTarget").contentWindow.document.getElementById("upload");
					picker.onchange=function() {
						...
						var result = filePickerEditor.fire("pptuploads.startUpload", evdata );
						// in v3 cancel() returns true and in v4 returns false
						// if not canceled it's the evdata object
						if ( typeof result == "boolean" )
							return;

						if (evdata.requiresImage && !CKEDITOR.plugins.pptuploads.isImageExtension(filePickerEditor, evdata.name))
						{
							errormsg_dlg(filePickerEditor.lang.pptuploads.nonImageExtension, 'File Upload');
							return;
						}
			...
			editor.on( "destroy", function () {
				CKEDITOR.tools.removeFunction( this._.pptuploadsFormUploadFn );
				CKEDITOR.tools.removeFunction( this._.pptuploadsFormInitFn );
			} );
		}
	...
	if (!filePicker)
	{
		...
		filePicker.addEventListener("change", function ()
		{
			...
			for(var i=0; i<count; i++)
			{
				...
				evData.id = CKEDITOR.plugins.pptuploads.getTimeStampId();
				...
				CKEDITOR.plugins.pptuploads.insertSelectedFile(filePickerEditor, evData);
			}
		});
	}
	...
	function setUrl( fileUrl, msg )
	{
		...
		editor.fire('pptuploads.endUpload', { name: IEUpload_fileName, ok: (!!fileUrl) } );
		...
		if ( fileUrl ) {
			...
			else
			{
				...
				element.on("load", function(e) {
					...
					editor.fire('pptuploads.finishedUpload', { name: IEUpload_fileName, element: element } );
				});
			...
			if (filePickerForceLink)
				filePickerEditor.fire("pptuploads.finishedUpload", { name: IEUpload_fileName, element: element } );
		}
...
function receivedUrl(fileUrl, data, editor, el, attribute)
{
	...
	editor.fire('pptuploads.finishedUpload', { name: data.name, element: el } );
}

CKEDITOR.plugins.add( "pptuploads",
{
	...
	onLoad : function()	{
		...
		var content = '.pptuploadsOverContainer {' + (CKEDITOR.config.pptuploads_containerover || 'box-shadow: 0 0 10px 1px #99DD99 !important;') + '} ' +
					'.pptuploadsOverDialog {' + (CKEDITOR.config.pptuploads_dialogover || 'box-shadow: 0 0 10px 4px #999999 inset !important;') + '} ' +
					'.pptuploadsOverCover {' + (CKEDITOR.config.pptuploads_coverover || 'box-shadow: 0 0 10px 4px #99DD99 !important;') + '} ';
	...
	init : function( editor ) {
		...
		if (typeof config.pptuploads_imageExtensions == "undefined")
			config.pptuploads_imageExtensions = "jpe?g|gif|png";
		...
		if (!config.filebrowserUploadUrl && !config.filebrowserImageUploadUrl)
		{
			if (window.console && console.log)
			{
				...
				console[ console.warn ? "warn" : "log" ]("The 'pptuploads' plugin now is disabled.");
			}
			return;
		}
		...
		{
			editor.addFeature( {
				allowedContent: 'img[!src,width,height];a[!href];span[id](pptuploadsTmpWrapper);'
			} );
		}
		...
		editor.on( "pptuploads.startUpload" , function(ev)
		{
			var editor = ev.editor,
				config = editor.config,
				file = ev.data && ev.data.file;
			if (config.pptuploads_maxFileSize &&
				file && file.size &&
				file.size > config.pptuploads_maxFileSize )
			{
				//alert( editor.lang.pptuploads.fileTooBig );
				errormsg_dlg( editor.lang.pptuploads.fileTooBig, 'File upload limit exceed' );
				ev.cancel();
			}
			var name = ev.data.name;
			if (config.pptuploads_invalidExtensions)
			{
				var reInvalid = new RegExp( "\.(?:" + config.pptuploads_invalidExtensions + ")$", "i");
				if ( reInvalid.test( name ) )
				{
					errormsg_dlg( editor.lang.pptuploads.invalidExtension, 'File Upload' );
					ev.cancel();
				}
			}
			if (config.pptuploads_acceptedExtensions)
			{
				var reAccepted = new RegExp( "\.(?:" + config.pptuploads_acceptedExtensions + ")$", "i");
				if ( !reAccepted.test( name ) )
				{
					errormsg_dlg( editor.lang.pptuploads.nonAcceptedExtension.replace("%0", config.pptuploads_acceptedExtensions), 'File Upload' );
					ev.cancel();
				}
			}
		});

		// Special listener that captures uploads of images and if there's some listener set for "pptuploads.localImageReady"
		...
		editor.on( 'pptuploads.startUpload' , function(ev) {
			...
			if (data.forceLink || !CKEDITOR.plugins.pptuploads.isImageExtension(editor, data.name))
				return;
			...
			if (!editor.hasListeners( 'pptuploads.localImageReady' ))
				return;
			...
			if (data.mode.type=="base64paste")
			{
				...
				var idTmp = CKEDITOR.plugins.pptuploads.getTimeStampId();
				data.result = "<span id='" + idTmp + "' class='pptuploadsTmpWrapper' style='display:none'>&nbsp;</span>";
				data.mode.id = idTmp;
			}
			...
			img.onload = function() {
				...
				var result = editor.fire('pptuploads.localImageReady', evData);
				...
				CKEDITOR.plugins.pptuploads.insertProcessedFile(ev.editor, evData);
			};
		...
		if (config.pptuploads_convertBmp)
			editor.on( 'pptuploads.localImageReady', convertToBmp);

		if (config.pptuploads_maximumDimensions)
			editor.on( 'pptuploads.localImageReady', checkDimension);


		// workaround for image2 support
		 editor.on( 'pptuploads.finishedUpload' , function(ev) {
				...
				html = html.replace( /<img(.*?) src="data:image\/.{3,4};base64,.*?"(.*?)>/g, function( img )
					{
						...
						var match = img.match(/"(data:image\/(.{3,4});base64,.*?)"/),
							...
							id = CKEDITOR.plugins.pptuploads.getTimeStampId();
		...
		var avoidBadUndo = function(e) {
			...
			if (root.$.querySelector( ".pptuploadsTmpWrapper") )
			{
		...
		editor.addCommand( 'addPPT', {
			exec: function( editor ) {
				PickAndSendFile(editor, false, this);
			}
		});

		editor.ui.addButton( 'addPPT', {
			label: editor.lang.pptuploads.addFile,
			command: 'addPPT',
			icon : this.path + 'icons/addfile.png', // %REMOVE_LINE_CORE%
			toolbar: 'insert',
			allowedContent : 'a[!href];span[id](pptuploadsTmpWrapper);',
			requiredContent : 'a[href]'
		});

		// Images
		/*
		editor.addCommand( 'addPPTImage', {
			exec: function( editor ) {
				PickAndSendFile(editor, true, this);
			}
		});

		editor.ui.addButton( 'addPPTImage', {
			label: editor.lang.pptuploads.addImage,
			command: 'addPPTImage',
			icon : this.path + 'icons/addimage.png', // %REMOVE_LINE_CORE%
			toolbar: 'insert',
			allowedContent : 'img[!src,width,height];span[id](pptuploadsTmpWrapper);',
...
*/
		...
		var removeBaseHighlight = function() {
			...
			if ( dialog )
			{
				...
				div.removeClass( 'pptuploadsOverCover' );
			}
			else
			{
				editor.container.removeClass( 'pptuploadsOverContainer' );
			}
		};

		editor.on( 'destroy', function() {
			CKEDITOR.removeListener( 'pptuploads.droppedFile', removeBaseHighlight);
		...
		CKEDITOR.on( 'pptuploads.droppedFile', removeBaseHighlight);
		...
		var CKEDITORdragenter = function(e) {
			if (mainMinX == -1)
			{
				...
				if ( dialog )
				{
					...
					var div = dialog.parts.title.getParent();
					div.addClass( 'pptuploadsOverCover' );
				}
				else
				{
					if (!editor.readOnly)
						editor.container.addClass( 'pptuploadsOverContainer' );
				}
		...
		var rootDropListener = function(e) {
			// editor
			visibleRoot.removeClass( "pptuploadsOverEditor" );
			minX = -1;
			...
			CKEDITOR.fire( 'pptuploads.droppedFile' );
			mainMinX = -1;
			...
			if ( data && data.files && data.files.length>0 )
			{
				...
				for( var i=0; i<data.files.length; i++)
				{
					...
					CKEDITOR.plugins.pptuploads.insertDroppedFile(editor, evData);
				}
			}
		};
		...
		var rootDragEnter = function(e) {
			if (minX == -1)
			{
				...
				if (!editor.readOnly)
					visibleRoot.addClass( "pptuploadsOverEditor" );
			...
			addTarget : function( target, callback )
			{
				target.on( 'dragenter', function(e) {
					if (minX == -1)
					{
						...
						target.addClass( 'pptuploadsOverDialog' );
				...
				target.on( 'dragleave', function(e) {
					...
					if ((ev.clientX<=minX) || (ev.clientY<=minY) || (ev.clientX>=maxX) || (ev.clientY>=maxY))
					{
						target.removeClass( 'pptuploadsOverDialog' );
						minX = -1;
					}
				});
				...
				target.on( 'drop', function(e) {
					...
					if ( data && data.files && data.files.length>0 )
					{
						...
						for( var i=0; i<1; i++)
						{
							...
							CKEDITOR.plugins.pptuploads.processFileWithCallback( editor, evData );
						}
					}
				});
			}
...
CKEDITOR.plugins.pptuploads = {
	...
	{
		if (!editor.config.pptuploads_imageExtensions)
			return false;

		var imageRegexp = new RegExp( "\.(?:" + editor.config.pptuploads_imageExtensions + ")$", "i");
...
{
	var isImage = CKEDITOR.plugins.pptuploads.isImageExtension( editor, data.name ),
		showImageProgress = !editor.config.pptuploads_hideImageProgress,
		element;
	...
	else
	{
		...
		element.setAttribute( "class", "pptuploadsTmpWrapper");
...
function checkLoadedImage(img, editor, el, name)
{
	...
	editor.fire('pptuploads.finishedUpload', { name: name, element: img } );
...
function createXHRupload(editor, data)
{
	...
	{
		errormsg_dlg(editor.lang.pptuploads.nonImageExtension, 'File Upload');
		return null;
	}

	var result = editor.fire("pptuploads.startUpload", data);
	// in v3 cancel() returns true and in v4 returns false
...
//			data.xhr = reader;
	...
	xhr.onload = function() {
		...
		var result = editor.fire("pptuploads.serverResponse", evtData);
		...
		editor.fire('pptuploads.endUpload', { name: data.name, ok: (!!fileUrl), xhr : xhr, data : data } );
		if (xhr.status!=200)
		{
			if (xhr.status == 413)
				errormsg_dlg( editor.lang.pptuploads.fileTooBig, 'File Upload' );
			else
...
function updateProgress(editor, id, evt)
{
	...
	{
		textValue = editor.lang.pptuploads.processing;
		value = "100%";
	/*
		if (text)
		{
...
function createSVGAnimation( file, id, editor )
{
	...
	function createSimpleUpload( editor, dialogName, definition, element )
	{
		...
		if (element.filebrowser.action == "QuickUpload") {
			...
			if (!editor.config.pptuploads_respectDialogUploads)
			{
				element.label = ( imageDialog ? editor.lang.pptuploads.addImage : editor.lang.pptuploads.addFile);
	...
	function processPastedFile(file, e)
	{
		...
		var id = CKEDITOR.plugins.pptuploads.getTimeStampId(),
		...
		CKEDITOR.plugins.pptuploads.insertPastedFile(editor, evData);
	}
	...
	{
		if (!evt.editor.plugins.pptuploads)
			return;
...
/**
 * Fired when file starts being uploaded by the "pptuploads" plugin
 *
 * @since 3.1
 * @name CKEDITOR.editor#pptuploads.startUpload
 * @event
 ...
 *
 * @since 4.3.6
 * @name CKEDITOR.editor#pptuploads.serverResponse
 * @event
 * @param {Object} [xhr] The XHR with the request.
 * @param {Object} [data] The original data object of this upload.
 * Upon processing this event, a listener can set a "url" property on the event.data object and that will tell to the pptuploads plugin
...
/**
 * Fired when file upload ends on the "pptuploads" plugin
 *
 * @since 3.1
 * @name CKEDITOR.editor#pptuploads.endUpload
 * @event
 * @param {String} [name] The file name.
...
/**
 * Fired when the final element has been inserted by the "pptuploads" plugin (after it has been uploaded)
 *
 * @since 3.3.4
 * @name CKEDITOR.editor#pptuploads.finishedUpload
 * @event
 * @param {String} [name] The file name.
 ...
 * that contains the selected file. Extends the data provided in the pptuploads.startUpload event
 * @since 4.2
 * @name CKEDITOR.editor#pptuploads.localImageReady
 * @event
 ...
 *
 *		CKEDITOR.config.pptuploads_containerover='border:1px solid red !important;';
 *
 * @since 2.7
 * @cfg {String} [pptuploads_containerover='box-shadow: 0 0 10px 1px #99DD99 !important;']
 ...
 *
 *		CKEDITOR.config.pptuploads_editorover='background-color:yellow !important;';
 *
 * @since 2.7
 * @cfg {String} [pptuploads_editorover='box-shadow: 0 0 10px 1px #999999 inset !important;']
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pptuploads_coverover='border:1px solid red !important;';
 *
 * @since 4.0
 * @cfg {String} [pptuploads_coverover='box-shadow: 0 0 10px 4px #999999 inset !important;']
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pptuploads_dialogover='border:1px solid red !important;';
 *
 * @since 4.0
 * @cfg {String} [pptuploads_dialogover='box-shadow: 0 0 10px 4px #999999 inset !important;']
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pptuploads_imageExtensions='jpe?g';
 *
 * @since 3.3.3
 * @cfg {String} [pptuploads_imageExtensions='jpe?g|gif|png']
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pptuploads_maxFileSize=10*1024*1024; // 10 Mb
 *
 * @since 3.3.3
 * @cfg {Number} [pptuploads_maxFileSize=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pptuploads_invalidExtensions='exe|php';
 *
 * @since 3.3.3
 * @cfg {String} [pptuploads_invalidExtensions=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pptuploads_acceptedExtensions='jpg|png|pdf|zip';
 *
 * @since 3.3.3
 * @cfg {String} [pptuploads_acceptedExtensions=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.pptuploads_respectDialogUploads=true;
 *
 * @since 4.0
 * @cfg {Boolean} [pptuploads_respectDialogUploads=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pptuploads_hideImageProgress=true;
 *
 * @since 4.1
 * @cfg {Boolean} [pptuploads_hideImageProgress=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pptuploads_convertBmp=true;
 *
 * @since 4.2
 * @cfg {Boolean} [pptuploads_convertBmp=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		CKEDITOR.config.pptuploads_maximumDimensions={width:500, height:400};
 *
 * @since 4.2
 * @cfg {Object} [pptuploads_maximumDimensions=null]
 * @member CKEDITOR.config
 */
